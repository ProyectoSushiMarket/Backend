...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CREAR_PRODUCTO` (IN `_NOMBRE` VARCHAR(100), IN `_UNIDADDEMEDIDA` VARCHAR(50), IN `_PRECIO` DECIMAL(10,3), IN `_IMAGEN` TEXT)   BEGIN

INSERT INTO productos(nombre, unidad_de_medida, precio, imagen) VALUES (_NOMBRE, _UNIDADDEMEDIDA, _PRECIO, _IMAGEN);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MODIFICAR_PRODUCTO` (IN `_NOMBRE` VARCHAR(100), IN `_NOMBRE_NUEVO` VARCHAR(100), IN `_UNIDADMEDIDA` VARCHAR(50), IN `_PRECIO` DECIMAL(10,3), IN `_IMAGEN` TEXT)   BEGIN
    UPDATE productos 
    SET 
...
(56, 'Cebolla Original 2.0', 'GR', 0.000, 'qawsedr'),
